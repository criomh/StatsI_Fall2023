...
for(j in 1:length(cor.level)){ # Start the j loop
  for(i in 1:reps){ # Start the loop
    #i <- 1
    #j <- 1 # [sets degree of correlation ie which row/column to match. If want to do everything, just remove these two lines and the whole matrix will be filled in]
    ...
    X2 <- X[ , 2] # [up to here, we are just preparing data for the simulation]
    Y <- b0 + b1*X1 + b2*X2 + rnorm(n, 0, 1) # The true DGP, with N(0, 1) error [rnorm(...) adds the "stocastic error" - note determinist ontology doesn't believe this exists ie "all error is systematic. I just need a perfect model and that will rid of this" vs probabalistic ontology ie "it isn't possible to have a 1-1 perfect model, the world is not purely deterministic"]
...
mean(par.est.ov[ , 11]) # r=0.99 # The more the two variables covary (i.e. higher correlation), the higher the systematic error (and the further the mean is from the true value). Therefore, we want to "control" for variables which highly correlate with BOTH predictor and response variable (ie confounding/mediating/etc)
